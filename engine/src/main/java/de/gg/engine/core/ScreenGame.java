 ...
 */
public abstract class ScreenGame extends Game {
	 ...
	 */
	private Map<String, LoadableScreen> screens = new ConcurrentHashMap<>();
	 ...
	 *            the screen.
	 */
	public final void addScreen(String name, LoadableScreen screen) {
	...
	protected abstract <S extends LoadableScreen> void onScreenInitialization(
			S screen);
	...
	public synchronized final void pushScreen(String name) {
		...
		LoadableScreen pushedScreen = screens.get(name);
		...
		Gdx.app.postRunnable(() -> {
			...
			if (!pushedScreen.isLoaded()) {
				assetManager.load(pushedScreen.getClass());
				assetManager.finishLoading();
				assetManager.injectAssets(pushedScreen);
	 ...
	 */
	public final LoadableScreen getScreen(String name) {
		Preconditions.checkArgument(!name.isEmpty(), "name cannot be empty");

		LoadableScreen screen = this.screens.get(name);
