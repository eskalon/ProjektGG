...
public class GifDecoder {
	...
	private static class DixieMap extends Pixmap {
		...
		DixieMap(int[] data, int w, int h, Pixmap.Format f) {
			...
			for (y = 0; y < h; y++) {
				for (x = 0; x < w; x++) {
					...
					int pxl_RGBA8888 = ((pxl_ARGB8888 >> 24) & 0x000000ff)
							| ((pxl_ARGB8888 << 8) & 0xffffff00);
		...
		void getPixels(int[] pixels, int offset, int stride, int x, int y,
			...
			for (k = y; k < y + height; k++) {
				...
				for (l = x; l < x + width; l++) {
					...
					pixels[_offset++] = ((pxl >> 8) & 0x00ffffff)
							| ((pxl << 24) & 0xff000000);
				}
				offset += stride;
			}
	...
	public Animation<TextureRegion> getAnimation(PlayMode playMode) {
		...
		Pixmap target = new Pixmap(width * hzones, height * vzones,
		...
		for (h = 0; h < hzones; h++) {
			for (v = 0; v < vzones; v++) {
				int frameID = v + h * vzones;
				if (frameID < nrFrames) {
					TextureRegion tr = new TextureRegion(texture, h * width,
							v * height, width, height);
					texReg.add(tr);
				}
			}
		}
		...
		Animation<TextureRegion> result = new Animation<TextureRegion>(
				frameDuration, texReg, playMode);
