...
public class MicroOptions {
	...
	public class Option {
		 ...
		 * @return this option
		 */
		public Option describedAs(String d) {
			this.d = d;
			return this;
		}
	...
	private final java.util.Map<String, Option> opts = new java.util.TreeMap<String, Option>();
	// All option values that have been parsed.
	private final java.util.Map<String, String> args = new java.util.HashMap<String, String>();
	...
	public void parse(String... strings) {
		for (int i = 0; i < strings.length; i++) {
			...
			if (k.matches("-[[^\\s]&&[^-]]")) {
				k = k.substring(1);
			} else if (k.matches("--[\\S]{2,}")) {
				k = k.substring(2);
			} else
				throw new UnsupportedOptionException(k);
			if (!opts.containsKey(k))
				throw new UnsupportedOptionException(k);
			Option o = opts.get(k);
			if (!o.u) {
				if (i + 1 >= strings.length)
					throw new MissingArgException(k);
				value = strings[++i];
			}
			args.put(k, value);
		}
	...
	/**
	 * Configure an optional, no-arg option having the specified name. The name
	 ...
	 *             if the name is null, the empty string, or starts with a
	 *             hyphen
	 ...
	 *             if the option hasn't been defined via
	 *             {@link #option(String)}; also thrown if the name passed to
	 *             the method is null, the empty string, or starts with a hyphen
	 */
	 ...
	 * @throws UnsupportedOptionException
	 *             if the option hasn't been defined via {@link #option(String)}
	 *             or is unary (cannot have an argument); also thrown if the
	 *             name passed to the method is null, the empty string, or
	 *             starts with a hyphen
	 */
	public String getArg(String option) {
		return getArg(option, null);
	}
	 ...
	 *            the option to look up the argument for
	 * @param defaultValue
	 *            the value to return if the specified option was not
	 *            encountered during parsing
	 * @return the argument, iff the option takes and argument and was not
	 *         encountered during parsing; otherwise, the specified default
	 *         value
	 * @throws UnsupportedOptionException
	 *             if the option hasn't been defined via {@link #option(String)}
	 *             or is unary (cannot have an argument); also thrown if the
